...
const SetsVennVisualizer = () => {
  ...
  const getHighlightClass = (num: number) => {
    const inA = setA.includes(num);
    const inB = setB.includes(num);
    const inResult = result.includes(num);

    if (!inResult) return 'opacity-30';
    if (inA && inB) return 'ring-2 ring-warning';
    if (inA) return 'ring-2 ring-primary';
    if (inB) return 'ring-2 ring-secondary';
    return '';
  };
